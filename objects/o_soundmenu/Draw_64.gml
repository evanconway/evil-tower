...
if (global.soundmenuactive) {
	...
	v_soundmenu_rectx = global.resolution_width/2 - v_soundmenu_rectwidth/2;
	v_soundmenu_recty = global.resolution_height/2.5 - v_soundmenu_rectheight/2;
	
	//menu panel
	
	var x2 = v_soundmenu_rectx + v_soundmenu_rectwidth;
	var y2 = v_soundmenu_recty + v_soundmenu_rectheight;
	var borderx = v_soundmenu_border;
	var bordery = v_soundmenu_border;
	...
	for (i = 0; i < v_soundmenu_items; i++) {//remember that this is drawing bottom up
		...
		var xx = global.resolution_width/v_soundmenu_x;
		var yy = global.resolution_height/v_soundmenu_y - v_soundmenu_itemheight * i * v_soundmenu_spacer;
		draw_set_color(color);
		draw_set_alpha(1);
		draw_text_transformed(global.resolution_width/v_soundmenu_pointerxxdivide, yy, cursor, 1, 1, 0);
		draw_text_transformed(xx, yy, txt, 1, 1, 0);
		
		//effects
		if (i == 1) draw_text_transformed(global.resolution_width/v_soundmenu_valuex, yy, string_format(global.sfx_volume * 10, 1, 0), 1, 1, 0);
		//music
		if (i == 2) draw_text_transformed(global.resolution_width/v_soundmenu_valuex, yy, string_format(global.music_volume * 10, 1, 0), 1, 1, 0);
