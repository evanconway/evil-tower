...
if (actcon.v_actcon_pressed_down && place_meeting(ID.x, ID.y, o_ladder)) result = true;

// Actors need to be able to climb down ladders when they're on top of a wall with a ladder descending it.
if (actcon.v_actcon_pressed_down && place_meeting(ID.x, ID.y + 1, o_ladder)) result = true;
...
for (var i = 0; i <= ID.bbox_right - bbox_left; i++) {
	if (position_meeting(ID.bbox_right + i, ID.bbox_bottom + 1, o_ladder)) laddertouched = true;
	if (position_meeting(ID.bbox_right + i, ID.bbox_bottom + 1, o_wall)) walltouched = true;
}
