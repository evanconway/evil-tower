...
for (var chunk_i = 0; chunk_i < ds_list_size(ID.v_textbox_chunks); chunk_i++) {
	var segments = ds_list_find_value(ID.v_textbox_chunks, chunk_i);
	...
	for (var seg_i = 0; seg_i < ds_list_size(segments); seg_i++) {
		...
		*/
		if (scr_get_width_segment_nospace(t_o_segment) >= max_segment_width) {
			...
			chunk_i = ds_list_size(ID.v_textbox_chunks);
		} else {
